...
window.autoSave = () => {
    ...
    if (typeof window.saveUserData === 'function') window.saveUserData(data);
};
...
window.addRow = (containerId, type) => {
    ...
    if (type === 'investment') {
        ...
        row.innerHTML = `<td class="px-4 py-3"><input type="text" oninput="window.autoSave()" placeholder="Account" class="bg-transparent outline-none w-full text-sm"></td>
            ...
            <td class="px-4 py-3 text-right"><input type="number" oninput="window.autoSave()" placeholder="Total Balance" class="w-full text-right font-bold outline-none bg-transparent text-sm">
                <div class="cost-basis-container hidden"><label class="text-[8px] font-bold text-indigo-400 uppercase block">Cost Basis</label><input type="number" oninput="window.autoSave()" placeholder="Basis" class="w-full text-right text-[10px] text-indigo-400 outline-none bg-transparent"></div></td>
        ...
        const row = document.createElement('tr');
        row.className = "border-t border-slate-100";
        row.innerHTML = `<td class="px-4 py-3"><input type="text" oninput="window.autoSave()" placeholder="Asset Name" class="bg-transparent outline-none w-full text-sm"></td>
        ...
        const row = document.createElement('tr');
        row.className = "border-t border-slate-100";
        row.innerHTML = `<td class="px-4 py-3"><input type="text" oninput="window.autoSave()" placeholder="Property Name" class="bg-transparent outline-none w-full text-sm font-bold text-slate-700"></td>
        ...
        div.innerHTML = `<button onclick="this.parentElement.remove(); window.autoSave()" class="absolute top-2 right-2 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><i class="fas fa-times text-xs"></i></button>
            <div class="flex flex-wrap gap-4 items-center">
                ...
                <div class="flex flex-col"><label class="text-[8px] font-bold text-slate-400 uppercase">Base Salary</label><div class="flex items-center gap-1"><span class="text-slate-400 text-xs">$</span><input type="number" oninput="window.autoSave()" placeholder="0" class="w-28 bg-white border border-slate-200 p-1.5 rounded font-bold text-right text-sm outline-none"></div></div>
                <div class="flex flex-col"><label class="text-[8px] font-bold text-slate-400 uppercase">Bonus %</label><div class="flex items-center gap-1"><input type="number" oninput="window.autoSave()" placeholder="0" class="w-16 bg-white border border-slate-200 p-1.5 rounded font-bold text-right text-sm outline-none"><span class="text-slate-400 text-xs">%</span></div></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="flex flex-col justify-center"><div class="flex justify-between text-[9px] font-bold text-slate-500 mb-1 uppercase"><span>Annual Increase</span><span class="text-indigo-600">3.5%</span></div><input type="range" min="0" max="10" step="0.5" value="3.5" oninput="this.previousElementSibling.lastElementChild.innerText = this.value + '%'; window.autoSave()" class="w-full"></div>
                <div class="flex flex-col gap-2"><div class="flex justify-between text-[9px] font-bold text-slate-500 uppercase"><span>401k Personal</span><span class="text-indigo-600">0%</span></div><input type="range" min="0" max="30" step="1" value="0" oninput="this.previousElementSibling.lastElementChild.innerText = this.value + '%'; window.autoSave()" class="w-full">
                    <label class="flex items-center gap-2 text-[10px] text-slate-500"><input type="checkbox" onchange="window.autoSave()" checked> Include Bonus</label></div>
                <div class="flex flex-col gap-2"><div class="flex justify-between text-[9px] font-bold text-slate-500 uppercase"><span>401k Company</span><span class="text-indigo-600">0%</span></div><input type="range" min="0" max="20" step="0.5" value="0" oninput="this.previousElementSibling.lastElementChild.innerText = this.value + '%'; window.autoSave()" class="w-full">
                    <label class="flex items-center gap-2 text-[10px] text-slate-500"><input type="checkbox" onchange="window.autoSave()" checked> Include Bonus</label></div>
            </div>`;
        ...
        const row = document.createElement('tr');
        row.className = "border-t border-slate-100";
        row.innerHTML = `<td class="px-4 py-3"><input type="text" oninput="window.autoSave()" placeholder="Expense Name" class="bg-transparent outline-none w-full text-sm"></td>
